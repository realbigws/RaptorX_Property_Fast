...
void Usage(char *arg) 
{
	printf("Usage: %s -i input_tgt_file [ -o updated_tgt_file ] [ -t temporary_root ] \n",arg);
}
//---- parameter editor ----//
...
int main(int argc, char** argv)
{
	...
	char c = 0;
	int option_index=0;
	while ((c = getopt_long(argc, argv, "i:o:t:",long_options,&option_index)) != EOF) 
	{
		switch (c) 
		{
			case 'i':
...
//--- over ---//
	...
	if(output_tgt=="" || input_tgt==output_tgt)output_tgt=targetName+".tgt2";
	string sequence = "";
	char buf[BufLen];
...
{
	string wscommand;
	wscommand="util/TGT_Get_SEQ "+input_tgt+" "+temporary_root+"/"+targetName+".seq";
	system(wscommand.c_str());
	wscommand="util/TPL_TGT_Get_PSM "+input_tgt+" "+temporary_root+"/"+targetName+".mtx";
	system(wscommand.c_str());
	wscommand="util/TPL_TGT_Get_PSP "+input_tgt+" "+temporary_root+"/"+targetName+".psp";
	system(wscommand.c_str());
	wscommand="util/TPL_TGT_Get_HMM "+input_tgt+" "+temporary_root+"/"+targetName+".hhm";
	system(wscommand.c_str());
	wscommand="util/TGT_Get_SS2 "+input_tgt+" "+temporary_root+"/"+targetName+".ss2";
	system(wscommand.c_str());
}
...
// command to generate features 
	string genSS8_cmd = "util/genSS8_new.sh " + input_tgt + " " + temporary_root;
	system(genSS8_cmd.c_str());
	string genACC_cmd = "util/genACC_new.sh " + input_tgt + " " + temporary_root;
...
//	fin_combine = fin_combine + "util/combine_file.sh_old tmp/"+targetName+".ss2 tmp/"+targetName+".ss8 tmp/"+targetName+".acc "+output_tgt;
	fin_combine = fin_combine + "util/combine_file.sh_new "+temporary_root+"/"+targetName+".ss2 "+temporary_root+"/"+targetName+".ss8 "+temporary_root+"/"+targetName+".acc "+output_tgt;
